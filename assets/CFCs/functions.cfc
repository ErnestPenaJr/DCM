...
<cffunction name="getAllTasks" access="remote" returntype="any" returnformat="JSON">
    ...
    <cfset var retVal = ArrayNew(1)>
    <cfquery username="ErnestPenaJr" password="$268RedDragons" datasource="fellows_ledger" name="results">
        SELECT TASKID,TASK_NAME,TASK_DESCRIPTION,TASK_TIME,PROJECT,DEPTID,ALLOCATED_TiME,DATE
...
<cffunction name="addTasks" access="remote" returntype="any" returnformat="JSON">
    ...
    <cfquery username="ErnestPenaJr" password="$268RedDragons" datasource="fellows_ledger" name="insert">
        INSERT INTO MY_CAPACITY.DAILY_TASKS (TASK_NAME,TASK_DESCRIPTION,TASK_TIME,PROJECT,DEPTID,ALLOCATED_TiME,WEEK_NUM,DATE)
     ...
     <cfquery username="ErnestPenaJr" password="$268RedDragons" datasource="fellows_ledger" name="results">
        SELECT TASKID,TASK_NAME,TASK_DESCRIPTION,TASK_TIME,PROJECT,DEPTID,ALLOCATED_TiME,WEEK_NUM,DATE
        FROM MY_CAPACITY.DAILY_TASKS
        WHERE TASK_ID = MAX(TASK_ID)
...
<cffunction name="getTaskByWeek" access="remote" returntype="any" returnformat="JSON">
    ...
    <cfset var retVal = ArrayNew(1)>
    <cfquery username="ErnestPenaJr" password="$268RedDragons" datasource="fellows_ledger" name="results">
        SELECT WEEK_NUM, PROJECT, DAYOFWEEK(DATE) AS DAY_NUM, SUM(TASK_TIME) AS TOTAL_TIME
        FROM MY_CAPACITY.DAILY_TASKS
        WHERE WEEK_NUM = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.WEEK_NUM#" />
        GROUP BY WEEK_NUM, PROJECT, DATE
        ORDER BY WEEK_NUM ASC, DAY_NUM ASC
    </cfquery>
...
<cffunction name="getAllTasks" access="remote" returntype="any" returnformat="JSON">
    ...
    <cfset var retVal = ArrayNew(1)>
    <cfquery username="ErnestPenaJr" password="$268RedDragons" datasource="fellows_ledger" name="results">
        SELECT TASKID,TASK_NAME,TASK_DESCRIPTION,TASK_TIME,PROJECT,DEPTID,ALLOCATED_TiME,WEEK_NUM,DAYOFWEEK(DATE) AS DAY_NUM
        FROM MY_CAPACITY.DAILY_TASKS
        ORDER BY WEEK_NUM ASC, DAY_NUM ASC
    </cfquery>
